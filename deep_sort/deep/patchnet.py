...
class PatchNet(nn.Module):
    def __init__(self, class_num=1000, backbone=resnet50_laststride1, pretrained=True,
    ...
    def forward(self, x):
        ...
        if self.is_for_test is True:
            local_feat_list = torch.cat(local_feat_list, dim=1)
...
class PatchNetUn(PatchNet):
    def __init__(self, class_num=1000, backbone=resnet50_laststride1, last_stride=1, pretrained=True, is_for_test=False):
        super(PatchNetUn, self).__init__(class_num=class_num, backbone=backbone,
...
def patchnet_un(num_classes=1000, loss='softmax', **kwargs):
    return PatchNetUn(class_num=num_classes, is_for_test=True, last_stride=1)

def patchnet(num_classes=1000, loss='softmax', **kwargs):
    return PatchNet(class_num=num_classes, is_for_test=True, last_stride=1)